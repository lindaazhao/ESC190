...
int get_player_rank(char player_id[], PlayerRecord* root){
	return get_player_rank_helper(player_id, root, 0);
...
int get_player_rank_helper(char player_id[], PlayerRecord* curr, int depth){
	...
	if (curr->left_child){ // Check to make sure we haven't made it to a leaf
		left_rank = get_player_rank_helper(player_id, curr->left_child, depth+1);
	}
	else{
		left_rank = -1;
	}
	if (curr->right_child){ // Check to make sure we haven't made it to a leaf
